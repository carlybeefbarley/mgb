...
const AssetItems = ( { assets } ) => (
  <Card.Group style={_nowrapStyle}>
    {(!assets || assets.length === 0) &&
      <Segment basic>No assets yet</Segment>}
    {_.map(assets, a => <AssetCard classNames='mgb-assetcard-width' allowDrag={true} asset={a} key={a._id} />) }
    { assets && assets.length > 0 && <div data-tooltip={`Only the ${_showLimit} most recently edited assets are shown here`} style={{ minWidth: '8em', textAlign: 'center', margin: 'auto' }}>. . .</div>}
  </Card.Group>
);
