...
export default class EditActor extends React.Component {
  ...
  handleSave(reason, thumbnail) {
    this
      .props
      .handleContentChange(this.props.asset.content2, thumbnail, reason)
  }

  getTabs(databag) {
    const _makeContent = Element => (<Element
  ...
  */
  ...
  handleTemplateClick(e) {
    if (e.target.dataset.template) {
      ...
      this
        .props
        .handleDescriptionChange("Created from Template: " + templateName.replace(/^alTemplate/, ''))
  ...
  render() {
    ...
    const LayerValid = ({layerName, isValid}) => (isValid
      ? <strong>{layerName}: Yes&emsp;</strong>
      : <em style={{
        color: 'grey'
      }}>{layerName}: No&emsp;</em>)
      ...
      <div className='ui grid edit-actor'>
        ...
        <div id="mgbjr-edit-actor-layerValid">
          <LayerValid
            ...
            isValid={ActorValidator.isValidForBG(databag)}/>
          <LayerValid
            layerName='Active'
            isValid={ActorValidator.isValidForActive(databag)}/>
          <LayerValid
            layerName='Foreground'
            isValid={ActorValidator.isValidForFG(databag)}/>
        </div>
        ...
        {showTemplate && <Modal
          ...
          <Modal.Header>
            Choose a template for the type of Actor, then modify the detailed options in the
            Actor Editor
          </Modal.Header>
        ...
        {!showTemplate && <Tabs tabs={this.getTabs(databag)}/>
}
      </div>
    )
  }
