...
class NPCBehavior extends BaseForm {
      ...
      <div>
        <div className="column">
          <div className="ui inline">
            ...
            {data[responseChoice] && this.dropArea('Drop', dropsObjectOnChoice, 'actor')}
            {data[responseChoice] &&
              data[dropsObjectOnChoice] &&
              this.bool('Drop is persistent?', responseChoiceDropPersistsYN)}
          </div>
        </div>
        {data[responseChoice] &&
          this.dropArea(
            data[takeObjectTypeOnChoice] == '0' ? 'NPC takes item' : 'NPC requires item',
            takesObjectOnChoice,
            'actor',
            {
              title:
        ...
        {data[responseChoice] &&
          data[takesObjectOnChoice] &&
          this.bool('Require instead of taking item?', takeObjectTypeOnChoice)}
        {data[responseChoice] && this.text('Count', takesObjectCountOnChoiceNum, 'number')}

        {data[responseChoice] && this.text('NPC then says...', saysWhatOnChoice)}
  ...
  render() {
    const { movementType, talkText } = this.data
...
export default withStores({ videoStore })(NPCBehavior)
