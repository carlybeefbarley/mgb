  ...
  errorHandlers = {
    [ActorHelper.errorTypes.MISSING_ACTOR]: (err, i) => (
      <Segment key={i} as='div'>
        <Container><Icon color='orange' size='large' name='warning sign' />Unable to locate actor <em>{err.actor}</em></Container>
        <Segment.Group horizontal>
          <Segment>
            ...
            }}>Remove Actor from map</Button>
          </Segment>
          <Segment>
            <DropArea kind='actor' text='Drop Actor here to replace missing actor with new one' onChange={(val, asset) => {
    ...
    [ActorHelper.errorTypes.MISSING_IMAGE]: (err, i) => {
      ...
      return (
        <Segment key={i} as='div'>
          <Container><Icon color='orange' size='large' name='warning sign' />Unable to locate image for an actor <em>{err.actor}</em></Container>
          <Segment>
            <QLink to={`/assetEdit/actor/${user}/${name}`}><Button color="blue">Edit Actor {err.actor}</Button></QLink>
          </Segment>
        </Segment>
      )
    }
