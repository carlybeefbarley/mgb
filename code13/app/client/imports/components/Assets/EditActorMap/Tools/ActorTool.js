...
export default class ActorTool extends React.Component {
  ...
  get data () {
    ...
    const data = tss[this.activeTileset]
    return data
  }

  get activeTileset(){
    return this.map.activeTileset == 0 ? 1 : this.map.activeTileset
  }

  /* helpers */
  adjustCanvas () {
    const map = this.props.info.content.map
    const ts = map.data.tilesets[this.activeTileset]
  ...
  drawTiles () {
    ...
    const tss = map.data.tilesets
    const ts = tss[this.activeTileset]
    const ctx = this.ctx
