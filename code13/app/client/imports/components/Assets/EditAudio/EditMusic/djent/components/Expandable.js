...
class Expandable extends Component {
  ...
  constructor(props) {
    ...
    const lsValue =
      props.enableStateSave && window.localStorage
        ? window.localStorage.getItem(`expandable-${escape(props.title)}`) === 'true'
        : false
  ...
  componentWillUpdate = nextProps => {
    if (
      nextProps.isExpanded !== this.state.isExpanded &&
  ...
  render() {
    return (
      ...
      >
        <div
          className={`expandable__title ${this.props.titleClassName ? this.props.titleClassName : ''}`}
          onClick={this.onClick}
        >
          {this.props.title}
        </div>
        <div
          className={`expandable__body ${this.props.bodyClassName ? this.props.bodyClassName : ''}`}
          onClick={() => (this.isPristine = false)}
        >
          {this.props.children}
        </div>
      </div>
