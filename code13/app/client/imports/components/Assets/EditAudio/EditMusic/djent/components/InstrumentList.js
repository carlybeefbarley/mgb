...
export default class InstrumentList extends Component {
  onSoundToggle = event => {
    ...
    const currentValue = this.props.instruments
      ...
      .sounds.find(s => s.id === soundID).enabled
    const prop = 'enabled'
  ...
  render() {
    const instrumentViews = this.props.instruments.map((instrument, index) => {
              ...
              <ul className="sound-list">
                {sounds.map((sound, i) => (
                  <li
                    ...
                    className={`sound-list__item ${sound.enabled ? 'is-enabled' : ''}`}
                    key={i}
                  >
      ...
      return (
        <div className="u-mb2" key={index}>
          <div className="u-flex-row" key={index}>
            <h4>
              <span onClick={e => this.launchSettings(instrument)} style={{ width: '19px', height: '32px' }}>
...
const InstrumentSettingsPane = ({ instrument, actions }) => (
  <div className="u-mb1">
    <PitchController
      ...
      id={instrument.id}
      actions={{ updateInstrumentPitch: actions.updateInstrumentPitch }}
    />
  </div>
)
