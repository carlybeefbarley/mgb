...
class ShareController extends Component {
  ...
  generatePreset = () => {
    const preset = {
      ...
      settings: {
        ...
        instruments: this.props.instruments.map(instrument => {
          return {
            id: instrument.id,
    ...
    this.getShareableURL(preset).then(url => {
      if (url) {
  ...
  getShareableURL = preset => {
    ...
    if (shareableURL.length > 3000) alert('Cannot share - resulting URL is too long to be shared')

    return this.getShortURL(shareableURL)
  }

  getShortURL = url => {
    return gapi.client.urlshortener.url
      .insert({ longUrl: url })
  ...
  render = () => {
    ...
    return (
      <div className="">
        <button
          className={`button-primary u-flex-row ${this.state.isLoading ? '' : 'icon-is-hidden'}`}
