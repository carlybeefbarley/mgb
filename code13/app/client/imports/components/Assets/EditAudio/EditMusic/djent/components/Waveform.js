...
const colorScheme = [[0, 136, 170], [239, 108, 91], [166, 188, 24]]
...
const level = (x, y, w, h, targetColor) => {
  ...
  const draw = ctx => {
    if (oldState.y === state.y && oldState.color.filter((val, i) => val === state.color[i]).length === 3)
      return
    ctx.fillStyle = '#fff'
...
export default class Waveform extends Component {
  ...
  componentWillUpdate = nextProps => {
    ...
    if (
      ...
      this.props.currentSrc !== nextProps.currentSrc &&
      this.props.currentSrc.context
    ) {
  ...
  fixCanvasScale = () => {
    ...
    const backingStoreRatio = this.ctx.webkitBackingStorePixelRatio || this.ctx.backingStorePixelRatio || 1
  ...
  loop = t => {
    ...
    const currentTime = !this.props.isPlaying
      ? 0
      : currentSrc ? currentSrc.context.currentTime - this.startTime : false
  ...
  updateLevels = data => {
    ...
    const newData = this.levels.map((item, i) => {
