...
;(function() {
  ...
  MidiWriter.NoteEvent = function(fields) {
    ...
    if (Array.isArray(this.pitch)) {
            ...
            if (i == 0) {
              noteOn = new MidiWriter.NoteOnEvent({
                data: MidiWriter.numberToVariableLength(restDuration).concat([
                  ...
                  MidiWriter.getPitch(this.pitch[i]),
                  this.velocity,
                ]),
              })
            } else {
              ...
              noteOn = new MidiWriter.NoteOnEvent({
                data: [0, MidiWriter.getPitch(this.pitch[i]), this.velocity],
              })
      ...
      noteOn = new MidiWriter.NoteOnEvent({
        data: MidiWriter.numberToVariableLength(restDuration).concat([
          MidiWriter.constants.NOTE_ON_STATUS,
          MidiWriter.getPitch(this.pitch[i]),
          this.velocity,
        ]),
      })
      noteOff = new MidiWriter.NoteOffEvent({
        data: MidiWriter.numberToVariableLength(tickDuration).concat([
          MidiWriter.constants.NOTE_OFF_STATUS,
          MidiWriter.getPitch(this.pitch[i]),
          this.velocity,
        ]),
      })
  ...
  MidiWriter.ProgramChangeEvent = function(fields) {
    ...
    this.data = MidiWriter.numberToVariableLength(0x00).concat([
      MidiWriter.constants.PROGRAM_CHANGE_STATUS,
      fields.instrument,
    ])
  }
  ...
  MidiWriter.Writer = function(tracks) {
    ...
    var trackType =
      tracks.length > 1
        ? MidiWriter.constants.HEADER_CHUNK_FORMAT1
        : MidiWriter.constants.HEADER_CHUNK_FORMAT0
    var numberOfTracks = MidiWriter.numberToBytes(tracks.length, 2) // two bytes long

    // Header chunk
    this.data.push(
      new MidiWriter.Chunk({
