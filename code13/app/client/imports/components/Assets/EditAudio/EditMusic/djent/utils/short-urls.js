...
const handleGoogleAPI = () => {
  return new Promise((res, rej) => {
    ...
    const handleClientLoad = () => {
      ...
      window.gapi.client.load('urlshortener', 'v1').then(res)
    }
...
const getPresetFromData = data => {
  ...
  const decompressedData = data && data.length % 4 === 0 && decompress(data, { inputEncoding: 'Base64' })
  const preset = /[A-Za-z0-9+/=]/.test(decompressedData) ? JSON.parse(decompressedData) : undefined

  return preset
}

export { getPresetData, getPresetFromData, handleGoogleAPI }
