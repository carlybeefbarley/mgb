...
function DG_MEMBERPARENT(tern, srv, query, file) {
  ...
  var wordStart = tern.resolvePos(file, query.end), wordEnd = wordStart, text = file.text;
  ...
  var word = text.slice(wordStart, wordEnd), ignoreObj;
  ...
  if (objLit) {
    ...
    while (pathStart && (text.charAt(pathStart - 1) == "." || isIdentifierChar(text.charCodeAt(pathStart - 1)))) pathStart--;
