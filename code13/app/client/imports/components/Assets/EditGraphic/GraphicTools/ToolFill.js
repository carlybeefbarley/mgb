...
function floodfill(x,y,fillcolor,ctx,width,height,tolerance) {
  ...
  var Q = [];
  var i = (x+y*width)*4;
  var e = i, w = i, me, mw, w2 = width*4;   // e=east color-boundary;   w=west color-boundary
  ...
  while (Q.length > 0 && abort === false) {
    ...
    if (pixelCompareAndSet(i,targetcolor,fillcolor,data,length,tolerance)) {
      ...
      while (mw<w && pixelCompareAndSet(w-4,targetcolor,fillcolor,data,length,tolerance) && mw<(w-=4))
