...
class ThumbnailWithInfo extends React.Component {
  ...
  componentDidMount(){
    if(this.props.showDimensions && !this.state.dimensions){
      const {asset, assetId, owner, name, expires = SpecialGlobals.thumbnail.defaultExpiresDuration} = this.props
  ...
  render(){
    ...
    return <div style={{textAlign: 'center'}}>
      ...
      {this.state.dimensions &&
        <span>{this.state.dimensions.width}x{this.state.dimensions.height}</span>
      }
    </div>
  }
}
