...
class ResolveReportEntity extends React.Component {
  ...
  render () {
    ...
    return (
      (isSuperAdmin && currUser && entity.suFlagId ) ?
      ...
      <Popup
        ...
        <Form style={{minWidth: '25em'}}>
            ...
            Ban this permanently?
              <br/><em>
              (the {tableCollection} will show up as deleted by moderator)
              </em>
            <Divider hidden />
            <Button.Group>
              <Button
              value={ true }
              onClick={ () => _doResolveReportEntity(entity._id, true, this.state.modComments, tableCollection )}
              negative>
              Yes Ban
              </Button>
            <Button.Or/>
              <Button
              value={ false }
              onClick={ () => _doResolveReportEntity(entity._id, false, this.state.modComments, tableCollection )}
              positive>
              Don't Ban
              </Button>
            </Button.Group>
