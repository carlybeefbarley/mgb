    ...
    for (var hidx = 0; hidx < hits.length; hidx++)
    {
      ...
      if (MgbActor.alActorType_Player == t1)		// Note the guarantees offered by playFindAACollisions()... Player is *never* the 2nd tuple
      {
        ...
        case MgbActor.alActorType_NPC:
          ...
          if (aa2.inMelee())
            this.applyDamageToActor(aa1_idx, ap1, MgbActor.intFromActorParam(ap2.content2.databag.allchar.meleeDamageToPlayerNum))
          else
            this.applyDamageToActor(aa1_idx, ap1, MgbActor.intFromActorParam(ap2.content2.databag.allchar.touchDamageToPlayerNum), MgbActor.intFromActorParam(ap2.content2.databag.allchar.touchDamageAttackChance))
          // 2. Touch Damage to enemy (aa2) from player (aa1)
          if (aa1.inMelee())
            this.applyDamageToActor(aa2_idx, ap2,
                  Math.max(0, MgbActor.intFromActorParam(ap1.content2.databag.allchar.meleeDamageToNPCorItemNum) + inventory.equipmentMeleeDamageBonus))	// Can't be -ve damage, so use max(0, ...) 
          else
        ...
        case MgbActor.alActorType_Item:
          ...
          if (aa1.inMelee())
          {
            if (MgbActor.intFromActorParam(ap1.content2.databag.allchar.meleeDamageToNPCorItemNum) && 0 == MgbActor.intFromActorParam(ap2.content2.databag.item.pushToSlideNum))
              this.applyDamageToActor(aa2_idx, ap2, 
                Math.max(0, MgbActor.intFromActorParam(ap1.content2.databag.allchar.meleeDamageToNPCorItemNum) + inventory.equipmentMeleeDamageBonus)) // Can't do -ve damage...
          }
          else
          {
            if (MgbActor.intFromActorParam(ap1.content2.databag.allchar.touchDamageToNPCorItemNum) && 0 == MgbActor.intFromActorParam(ap2.content2.databag.item.pushToSlideNum))
              this.applyDamageToActor(aa2_idx, ap2, ap1.content2.databag.allchar.touchDamageToNPCorItemNum, MgbActor.intFromActorParam(ap1.content2.databag.allchar.touchDamageAttackChance))
          }
