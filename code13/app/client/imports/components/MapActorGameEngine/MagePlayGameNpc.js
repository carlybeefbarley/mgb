...
export default MagePlayGameNpc = {
  ...
  handleNpcResponse(choice) 
  {
    ...
    if (npcDialogActor != null && npcDialogActor.ACidx != null)
      ap = actors[npcDialogActor.ACidx]
    ...
    if (take)
    {
      ...
      if (!item || item.count < takeCount)
      {
        if (takeCount === 1)
          ...
          say = `You don't have enough ${take} to give me. I want ${takeCount}.`
        drop = null
        stayYN = 1
      }
      else
    ...
    if (drop && drop !== '')
    {
      this.playSpawnNewActor(drop, npcDialogActor.x, 
    ...
    if (say)
    {
      ...
      if (repeatQuestion)
        this.askNpcQuestion(npcDialogActor, ap, true)		// final "true" asks to queue this up
    }
    else
    {
      if (repeatQuestion)
