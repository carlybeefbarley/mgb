...
const WhatsNew = ({ currUser, asHidingLink }) => {
  const laterNewsAvailable =
    currUser && currUser.profile && currUser.profile.latestNewsTimestampSeen !== _latestRelTimestamp
  ...
  if (asHidingLink)
    return !laterNewsAvailable
      ? null
      : <QLink to="/whatsnew" title="Announcements of new features/fixes to MGB" className="fitted item">
