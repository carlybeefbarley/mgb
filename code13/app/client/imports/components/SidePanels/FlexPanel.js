...
import FlexPanelContent from './FlexPanelContent'
...
export default FlexPanel = React.createClass({
  ...
  componentDidMount: function () {
    ...
    this.handleUp = () => {
      setTimeout(() => this.isDown = false, 1000)
    }
  ...
  _getSelectedFlexPanelChoice: function () {
    const views = this.props.fpIsFooter ? flexMobilePanelViews : flexPanelViews
    const defaultReturnValue = views[defaultPanelViewIndex]
    if (!this.props.selectedViewTag)
      return defaultReturnValue

    const selectedViewTagParts = this.props.selectedViewTag.split(".")
    // If the FlexPanel choice isn't recognized, just default to using our default one
    return _.find(views, ['tag', selectedViewTagParts[0]]) || defaultReturnValue
  },

  _getSelectedFlexPanelChoicePrevious: function (current) {
    const views = this.props.fpIsFooter ? flexMobilePanelViews : flexPanelViews
    const index = _.findIndex(views, current)
    return views[index - 1]
  },
  _getSelectedFlexPanelChoiceNext: function (current) {
    const views = this.props.fpIsFooter ? flexMobilePanelViews : flexPanelViews
    const index = _.findIndex(views, current)
    return views[index + 1]
  },
  ...
  handleChangeIndex(idx, old) {
    if(!this.isDown){
      return
    }
    console.log("Handle change:", idx, old, "Down?", this.isDown)

    if(old !== 1)
      return
    
    const c = this._getSelectedFlexPanelChoice()

    let btn
    if (idx < 1)
      btn = this._getSelectedFlexPanelChoicePrevious(c)

    if (idx > 1)
      btn = this._getSelectedFlexPanelChoiceNext(c)

    console.log("index changed: ", idx, btn);
    if (!btn)
      return

    this.fpViewSelect(btn.tag)
  },
  render: function () {
...
/*    top: 50px;
    ...
    const commonProps = {
      fpIsFooter: fpIsFooter,
      meteorStatus: this.props.meteorStatus,
      hazUnreadChat: hazUnreadChats,
      ...
      hazUnreadChats: this.props.hazUnreadChats,
    ...
    const first = fpIsFooter ? <FlexPanelContent flexPanelChoice={this._getSelectedFlexPanelChoicePrevious(flexPanelChoice)}
      {...commonProps}
    /> : null
    const current =  <FlexPanelContent flexPanelChoice={flexPanelChoice}
      {...commonProps}
    />
    const last = fpIsFooter ? <FlexPanelContent flexPanelChoice={this._getSelectedFlexPanelChoiceNext(flexPanelChoice)}
      {...commonProps}
    /> : null

    const cIndex = first ? 1 : 0
    ...
    return (
      <div className="basic segment mgbFlexPanel" style={panelStyle} id='mgbjr-flexPanelArea'>
        ...
        <div id='mgbjr-flexPanelIcons' className={miniNavClassNames} style={miniNavStyle} >
          {fpIsFooter && flexMobilePanelViews.map((v, i) => {
            if (i > 5) {
              return null
            }
          ...
          {!fpIsFooter && flexPanelViews.map(v => {  /* TODO: WORK OUT HOW TO HANDLE 5 equally space buttons */
            const active = this._viewTagMatchesPropSelectedViewTag(v.tag) ? " active selected " : ""
