...
import { withStores, withMeteorData } from '/client/imports/hocs'
...
class AssetEditRoute extends React.Component {
  ...
  getActivitySnapshots = () => this.state.data.activitySnapshots
  ...
  revertDataFromForkParent_ResultCallBack = (error, result) => {
    if (error) showToast.error(`Unable to revert content to ForkParent for this asset: '${error.toString()}'`)
      ...
      logActivity('asset.fork.revertTo', "Reverted to Fork Parent's data", null, this.state.data.asset)
    }
  ...
  componentDidMount() {
    ...
    this.assetHandler = this.state.data.assetHandler
  ...
  componentDidUpdate() {
    ...
    if (!this.counter && !this.state.data.loading) {
      this.assetUpdatedAt = this.state.data.asset.updatedAt
      this.counter = new EditTimeCounter(this.state.data.asset, this.props.currUser, newTime => {
    ...
    if (this.counter && this.state.data.asset && this.assetUpdatedAt !== this.state.data.asset.updatedAt) {
      this.assetUpdatedAt = this.state.data.asset.updatedAt
      this.counter.assetUpdated()
    }

    // auto open any new asset
    if (this.state.data.asset) {
      const { assetStore } = this.props
      // console.log('componentDidUpdate: open asset', this.state.data.asset, assetStore.getOpenAssets())
      assetStore.openAsset(this.state.data.asset)
    }
  }
  ...
  canCurrUserEditThisAsset = (assetOverride = null) => {
    const asset = assetOverride || this.state.data.asset
  ...
  canUserEditThisAssetIfUnlocked = (assetOverride = null) => {
    if (this.state.data.loading || !this.props.currUser) return false // Need to at least be logged in and have the data to
    // do any edits!
  ...
  canCurrUserChangeCompletion = (assetOverride = null) => {
    ...
    if (!asset || this.state.data.loading || !currUser) return false // Need to at least be logged in and have the data to do
    // any edits...
  ...
  forkResultCallback = (error, result) => {
    if (error) showToast.error(`Unable to create a forked copy of this asset: '${error.toString()}'`)
      ...
      logActivity('asset.fork.from', 'Forked new asset from this asset', null, this.state.data.asset)
  ...
  handleTabChange = asset => {
    ...
    return () => {
      utilPushTo(this.props.location, url)
    }
  }
  ...
  handleCloseTab = asset => {
    return e => {
      ...
      if (params.assetId === asset._id) {
        ...
        utilPushTo(this.props.location, nextURL)
      }
  ...
  render() {
    ...
    const assetHasProjects =
      ...
      this.state.data.asset !== null &&
      this.state.data.asset.projectNames.length > 0
  ...
  renderRoute = () => {
    if (this.state.data.loading) return <Spinner loadingMsg="Loading Asset data" />
    ...
    let asset = Object.assign({}, this.state.data.asset) // One Asset provided via getMeteorData()
    // console.log('Rendered Asset: ', asset)
    if (!this.state.data.asset) return <ThingNotFound type="Asset" id={params.assetId} />
        ...
        !isGuest && (
          <Grid.Column width="8" textAlign="right" id="mgbjr-asset-edit-header-right">
            ...
            <AssetHistoryDetail
              asset={asset}
              currUser={currUser}
              assetActivity={this.state.data.assetActivity}
          ...
          >
            <AssetEdit
              ...
              useExperimentalEditors={
                this.props.location.query[urlMaker.queryParams('app_experimentalEditors')]
              }
  ...
  deferContentChange = (content2Object, thumbnail, changeText = 'content change') => {
    const asset = this.state.data.asset // TODO: Change interface so this gets passed in instead?
  ...
  _attemptToSendAnyDeferredChanges = (options = {}) => {
    const asset = this.state.data.asset
    if (this.m_deferredSaveObj) {
  ...
  _sendContentChange = (assetId, content2Object, thumbnail, changeText = 'content change') => {
    ...
    if (content2Object) {
      if (this.state.data.asset && this.state.data.asset._id && this.state.data.asset._id !== assetId) {
        console.error('_doSendDeferredChangeNow() Detected overdue asset save. NOT Updating current asset.')
        this.m_deferredSaveObj = null
      } else this.state.data.update(updateObj)
    }
    logActivity('asset.edit', changeText, null, this.state.data.asset || { _id: assetId })
  }
  ...
  handleAssetDescriptionChange = newText => {
    if (newText !== this.state.data.asset.text) {
      Meteor.call(
        'Azzets.update',
        this.state.data.asset._id,
      ...
      logActivity('asset.description', `Update description to "${newText}"`, null, this.state.data.asset)
    }
  }
  ...
  handleMetadataChange = newMetadata => {
    ...
    logActivity('asset.metadata', `Update metadata of asset`, null, this.state.data.asset)
  }

  // This should not conflict with the deferred changes since those don't change these fields :)
  handleAssetNameChange = newName => {
    if (newName !== this.state.data.asset.name) {
      Meteor.call(
        'Azzets.update',
        this.state.data.asset._id,
        this.canCurrUserEditThisAsset(),
      ...
      logActivity('asset.rename', `Rename to "${newName}"`, null, this.state.data.asset)
    }
  ...
  handleLicenseChange = newLicense => {
    const oldLicense = this.state.data.asset.assetLicense
  ...
  handleWorkStateChange = newWorkState => {
    const oldState = this.state.data.asset.workState
  ...
  handleChatClick = () => {
    ...
    utilShowChatPanelChannel(this.props.location, channelName)
  }
  ...
  handleTaskApprove = hasSkill => {
    const { asset } = this.state.data
    const skillPath = this.state.data.asset.skillPath
