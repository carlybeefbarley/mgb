...
export default ProjectCreateNewRoute = React.createClass({
  ...
  render: function() {
    const numUserOwnprojects = _.filter(this.props.currUserProjects, p =>(p.ownerId === this.props.currUser._id)).length
    const maxUserOwnProjects = isUserSuperAdmin(this.props.currUser) ? SpecialGlobals.quotas.SUdefaultNumOfOwnedProjectsAllowed : SpecialGlobals.quotas.defaultNumOfOwnedProjectsAllowed
    return (
      <Segment basic>
        <Helmet
        ...
        <p style={(numUserOwnprojects >= maxUserOwnProjects) ? {color: 'red'} : undefined}> {`You currently own ${numUserOwnprojects} out of ${maxUserOwnProjects} projects allowed`}</p>
        <ProjectCreateNew 
