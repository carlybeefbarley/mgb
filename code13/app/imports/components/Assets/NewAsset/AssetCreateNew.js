...
export default AssetCreateNew = React.createClass({
  ...
  render: function() {
    ...
    const choices = AssetKindKeys.map((k) => {
      ...
      return  <div className={"ui " + (ak.disable ? "disabled ": "") + "link item"} key={k} style={sty} onClick={this.handleSelectAssetKindClick.bind(this,k)}>
                ...
                <div className="content" >
                  ...
                  <div className="description">{ak.description}</div>
                </div>
              </div>
    });

    // Calculate stuff for the 'Create Asset' button
    ...
    return (
      <div>
        ...
        <div className="ui padded segment">
          <h4 className="ui header">Choose Asset Kind</h4>        
          <div className="ui items">
            {choices}
          </div>
        </div>
