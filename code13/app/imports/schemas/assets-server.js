...
Meteor.methods({
  ...
  "Project.Azzets.fork": function (opts = {}) {
    ...
    if (conflictingProject)
      throw new Meteor.Error(402, 'A project already exists with that name')
  ...
  "Azzets.fork": function (srcId, opts = {}) {
    ...
    console.log(`  [Azzets.fork]  "${dstAsset.name}"  #${newForkedAssetId}  Kind=${dstAsset.kind}  Owner=${username}`)
  ...
  "Azzets.revertDataFromForkParent": function (assetId, forkParentId) {
    ...
    if (asset.ownerId !== this.ownerId)
      throw new Meteor.Error(401, "for now, only Asset Owners can revert-to-fork on an Asset. We will implement reverting an Asset as a Project member when it's needed")
    ...
    if (count !== 1)
      throw new Meteor.Error(`Azzets.update() failed to revert Asset #${assetId}`)
