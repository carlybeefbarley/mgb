...
var ConsoleLogViewer = (function() {
	...
	{
		if (!ConsoleLogViewer.logEnabled) 
      return;
    ...
    var content = noColor0 + "  " + this.flatten(args.join(", "));
		...
		while (_items.length > ConsoleLogViewer.TOTAL) 
      _items.shift();
	...
	{
		var css = '#debug_console { background: rgba(0,0,0,.75); font: 11px Arial, sans-serif!important; position:fixed; padding:0; margin:0; z-index:12834567; box-sizing:border-box; pointer-events:none; text-align:left; text-transform:none; }';
		...
		css += '#debug_console a.log-button {font: bold 11px Arial, sans-serif!important; pointer-events:all; text-align:center; text-decoration:none; border:1px solid #999; background:#333; color:#fff; width:14px; height:14px; padding:5px; margin:1px; display:block; float:right; }';
