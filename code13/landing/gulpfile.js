...
const meteorBuildPath = (...paths) =>
  makePath('../app/.meteor/local/build/programs/web.browser/', ...paths)
  ...
  through2.obj(function(file, enc, cb) {
    ...
    assetPaths.forEach(assetPath => {
      ...
      try {
        ...
        err.message =
          g.util.colors.red(`Could not copy asset "${assetPath}".\n`) +
          err.message
        this.emit('error', err)
      }
    })
...
const interpolateEnvVars = () =>
  through2.obj(function(file, enc, cb) {
    ...
    const interpolated = contents.replace(
      /\{\{(.*?)\}\}/g,
      (match, p1) => process.env[p1.trim()]
    )
...
gulp.task('publish', cb => {
  runSequence('clean', 'build', 'publish:make-cdn-files', 'publish:aws', cb)
})
...
gulp.task('publish:aws', function() {
  ...
  if (!AWS_CLOUDFRONT_DISTRIBUTION_ID)
    throw new Error(
      'Missing AWS_CLOUDFRONT_DISTRIBUTION_ID env var, see README.md'
    )
...
gulp.task('publish:make-cdn-files', () => {
  const { MGB_LANDING_BASE_URL, MGB_LANDING_HOST } = process.env
...
gulp.task('build', cb => {
  runSequence(
    ['build:fonts', 'build:html', 'build:images', 'build:js', 'build:styles'],
    cb
  )
})
